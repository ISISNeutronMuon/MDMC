...
if TYPE_CHECKING:
    ...
    from MDMC.MD import KSpaceSolver
...
class InteractionFunction:
    ...
    def __str__(self) -> str:
        ...
        parameters = ", ".join(f"{p.name}: {p.value}" for p in self.parameters.values())
    ...
    def __eq__(self, other):
        ...
        return self is other or (
            ...
            and all(
                a.type == b.type and a.value == b.value
                for a, b in zip(self.parameters.values(), other.parameters.values())
            )
        )
...
class HarmonicPotential(InteractionFunction):
    ...
    **settings
    ...
    def __new__(cls, equilibrium_state: float, potential_strength: float, **settings: Any):
        ...
        if "interaction_type" not in settings:
            raise TypeError('An interaction_type of "bond", "angle" or "improper" must be passed')

        match settings["interaction_type"].lower():
            case "bond":
                eq_unit = units.LENGTH
                pot_unit = units.ENERGY / units.LENGTH**2
            case "angle" | "bondangle" | "improper":
                eq_unit = units.ANGLE
        ...
        cls.__init__ = inter_func_decorator(
            ("equilibrium_state", eq_unit), ("potential_strength", pot_unit)
        )(cls._init)
        return h_pot
...
class LennardJones(InteractionFunction):
    ...
    **settings
    ...
    def __init__(
        ...
        long_range_solver: KSpaceSolver | None = None,
    ):

        super().__init__({"epsilon": epsilon, "sigma": sigma})
        self.cutoff = cutoff
        self.solver = long_range_solver
