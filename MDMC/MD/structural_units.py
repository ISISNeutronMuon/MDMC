...
import MDMC.MD.interactions as interactions
            ...
            if k == 'ID':
                ...
                for atom in atoms:
                    ...
                    for inter in atom.interactions:
                        if issubclass(type(inter), interactions.BondedInteraction):
                            memo[id(inter)] = inter
...
class Atom(StructuralUnit):
    ...
    **settings
        ...
        for k, v in self.__dict__.items():
            if k == 'ID':
                ...
                for inter in self.nonbonded_interactions:
                    if isinstance(inter, interactions.Coulombic):
    ...
    def charge(self):
        ...
        try:
            ...
            for interaction in self.interactions:
                if isinstance(interaction, interactions.Coulombic):
    ...
    def charge(self, value):
        ...
        if value is not None:
            interactions.Coulombic(atoms=self, charge=value, cutoff=10.)
    ...
    def atom_type(self, value):
        ...
        for inter in self.nonbonded_interactions:
            if isinstance(inter, interactions.Coulombic) and value not in inter.atom_types:
    ...
    def add_interaction(self, interaction, from_interaction=False):
        ...
        if issubclass(type(interaction), interactions.BondedInteraction):
