# MD
This package contains everything required to run a molecular dynamics simulation using an external engine.  This includes the modules for defining the simulation configuration and the topology, the subpackage for defining the force fields, and the subpackage in which the molecular dynamics engine facades are defined.  Converting an MD engine trajectory into an MDMC Trajectory is also contained within this package, although the Trajectory class is in the trajectory_analysis package.
