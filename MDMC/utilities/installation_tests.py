...
class InstlTestBase(ABC):
    ...
    def success(self) -> Literal["PASSED", "FAILED", "INCOMPLETE"]:
        ...
        if self._success is None:
            return "INCOMPLETE"
        raise ValueError(
            "The success value of this test has been incorrectly set. Please run the test again.",
        )
    ...
    def log_test_passed(self) -> None:
        ...
        LOGGER.info("%s %s installation test passed", self.__class__, self.name)
        ...
        for fso in fs_objects:
            ...
            if ".py" not in fso_base[-4:] and fso_base != "__pycache__":
                try:
                    ...
                    self._success = False
                    LOGGER.error(
                        "%s %s installation test failed because the"
                    ...
                    self._success = False
                    LOGGER.error(
                        "%s %s installation test failed because an"
                    ...
                    raise Exception(
                        "An Exception (other than an ImportError)"
...
class InstlTestLAMMPS(InstlTestBase):
    ...
    LOG_ERROR_MSG: str = (
        "Due to this, MDMC will not be able to run MD"
    ...
    def run(self) -> None:
        try:
            ...
            self._success = False
            LOGGER.error(
                "%s %s installation test failed because LAMMPS Python"
            ...
            self._success = False
            LOGGER.error(
                "%s %s installation test failed because the following"
        ...
        try:
            ...
            self._success = False
            LOGGER.error(
                "%s %s installation test failed because a PyLammps"
...
class InstlTestDynamicPlottingDependencies(InstlTestBase):
    ...
    def run(self) -> None:
        try:
            ...
            LOGGER.error(
                "%s %s installation test failed because %s could not"
