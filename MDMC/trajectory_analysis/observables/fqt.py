...
"""
A tutorial created for simulating pure methanol
"""

from MDMC.MD.interactions import Bond, BondAngle
from MDMC.MD import Atom, Dispersion, LennardJones, Simulation, Universe, Coulomb, Coulombic
...
# setting up initial size of the box
universe = Universe(dimensions=50.6, constraint_algorithm=Shake(1e-4, 100), electrostatic_solver=PPPM(accuracy=1e-5))
# setting up initial coordinates of atoms in methanol
C  = Atom('C', position=[0.6940, -0.0212, 0.0000], name='99', charge=0.1910, atom_type=2)
O  = Atom('O', position=[-0.7696, 0.1222, 0.0000], name='96', charge=-0.6700, atom_type=3)
H1 = Atom('H', position=[1.0530, -0.5452, -0.8967], name='98', charge=0.0330, atom_type=1)
H2 = Atom('H', position=[1.0530, -0.5452, 0.8967], name='98', charge=0.0330, atom_type=1)
H3 = Atom('H', position=[1.0818, 0.9993, 0.0000], name='98', charge=0.0330, atom_type=1)
H4 = Atom('H', position=[-1.1950, -0.7594, 0.0000], name='97', charge=0.4460, atom_type=4)
...
HCH_angle=BondAngle((H1, C, H2), (H2, C, H3), (H1, C, H3))
HCO_angle=BondAngle((H1, C, O), (H2, C, O), (H3, C, O))
COH_angle=BondAngle((C, O, H4))

H1COH4_dihedral = DihedralAngle((H1, C, O, H4))
H2COH4_dihedral = DihedralAngle((H2, C, O, H4))
H3COH4_dihedral = DihedralAngle((H3, C, O, H4))

H1_disp=Dispersion(universe, [1,1], cutoff=10., function=LennardJones(epsilon=0.0157, sigma=1.39), vdw_tail_correction=True)
H2_disp=Dispersion(universe, [1,1], cutoff=10., function=LennardJones(epsilon=0.0157, sigma=1.39), vdw_tail_correction=True)
H3_disp=Dispersion(universe, [1,1], cutoff=10., function=LennardJones(epsilon=0.0157, sigma=1.39), vdw_tail_correction=True)
C_disp=Dispersion(universe, [2,2], cutoff=10., function=LennardJones(epsilon=0.1094, sigma=1.91), vdw_tail_correction=True)
O_disp=Dispersion(universe, [3,3], cutoff=10., function=LennardJones(epsilon=0.2104, sigma=1.72), vdw_tail_correction=True)
H4_disp=Dispersion(universe, [4,4], cutoff=10., function=LennardJones(epsilon=0.00, sigma=0.00), vdw_tail_correction=True)

methanol = Molecule(atoms=[C, O, H1, H2, H3, H4])
...
meth_mol = Molecule(position=(0, 0, 0),
                     velocity=(0, 0, 0),
                     atoms=[C, O, H1, H2, H3, H4],
                     interactions=[Bond((C, H1), (C, H2), (C, H3), constrained=True),
                                   Bond((C, O), constrained=True),
                                   Bond((O, H4), constrained=True),
                                   BondAngle((H1, C, H2), (H3, C, H2), (H3, C, H1), constrained=True),
                                   BondAngle(C, O, H4, constrained=True),
                                   BondAngle((H1, C, O),(H2, C, O),(H3, C, O), constrained=True),
                                   DihedralAngle((H1, C, O, H4),(H2, C, O, H4),(H3, C, O, H4), constrained=True)],
                     name='methanol')
...
universe.fill(meth_mol, num_density=0.005)
view(universe)