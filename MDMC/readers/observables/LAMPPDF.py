...
class LAMPPDF(PDFReader):
    """
    Class for reading files from LAMP that contain PDF data.

    LAMP's ASCII output uses a single file, with the expected file
    structure being:
    ...
    The column file format above is the default with the total PDF
    data contained in the 3rd column.

    When initialising instances of this class it is possible to change
    this using the `pdf_col` parameter to select which data column
    contains the total PDF. The remaining columns (if they exist) are
    assumed to be partial PDFs.
    ...
    pdf_col : int, optional >= 3
        ...
        Default value is ``3`` as columns 1 and 2 are reserved for the
        row-counter and the distance value.
    partial_strings : list of tuple
        List of tuples to specify the labels of the partial
        pairs to be saved as such in
        ``PairDistributionFunction.partial_pdfs``.

        All columns in the data file apart from:

          - the row-counter (column 1),
          - distance values (column 2)
          - the one for the total PDF (`pdf_col`)

        are saved as `partial_pdfs`.

        The labels are applied in numerical order.

        If no labels are specified, the column header in the data
        file is used as the label.

    Attributes
    ----------
    r : ~numpy.ndarray
        The radial distance (in ``Ang``), expected in column 2 of the file.
    PDF : ~numpy.ndarray
        The total pair distribution function (in ``barn``),
        by default expected in column 3 of the file, but can be
        specified by `pdf_col` setting.
    partial_pdfs : dict[str, ~numpy.ndarray]
        The partial PDFs (in ``barn``),
        imported from the remaining columns with the labels
        of the partial pairs either specified by
        `partial_strings` or taken from the column headers.

    See Also
    --------
    MDMC.trajectory_analysis.observables.pdf.PairDistributionFunction : Main PDF container.
    """
    ...
    def assign(self, observable: PairDistributionFunction) -> None:
        """
        Assign the data parsed by the LAMPPDF reader to a PDF `Observable`.
        ...
        **settings
        ...
        try:
            ...
            msg = (f'The number of partial pair labels ({len(self.partial_strings)}) is not the '
                   ...
                   'This is either because the number of labels passed is incorrect or because '
                   'the column labels are not recognised correctly, e.g. due to an unexpected '
                   'delimiter.')
