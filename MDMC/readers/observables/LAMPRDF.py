...
class LAMPRDF(ObservableReader):
    ...
    def parse(self,**settings):
        ...
        self.PDF_err = np.zeros(self.PDF_err)
    ...
    def parse_indep_var(self, ffile):
        ...
        for i, line in enumerate(ffile1):
           if i==3:
             ...
             c_number[i-4]=int(column[0])
             # distance[i-4] or column[1] is the distance for which the rdfs were calculated.
             distance[i-4]=float(column[1])
        self.r=distance
        ffile1.close()
        return self.r
    ...
    def parse_dep_var(self, numrdf, ffile):
        ...
        rdf_err=np.zeros((num_rows,numrdf))
        c_number=np.zeros(num_rows)
        distance=np.zeros(num_rows) 
