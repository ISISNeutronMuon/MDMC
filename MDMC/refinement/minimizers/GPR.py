...
class GPR(Minimizer):
    ...
    def plot_results(
        ...
        if regressor_points.ndim != 2:
            raise ValueError(f"Fit results can only be plotted for systems with 2 parameters. Yours has {regressor_points.ndim}")
        ...
        ax.set_xticklabels(['']+["{:.5f}".format(l) for l in labels[0]])
        ax.set_yticklabels(['']+["{:.5f}".format(l) for l in labels[1]])
