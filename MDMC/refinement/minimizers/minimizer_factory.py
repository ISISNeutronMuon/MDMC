...
class MinimizerFactory:
    ...
    the minimizer folder can be created with a string of the class name, as
    long as it is a subclass of ``Minimizer``.
    """

    @staticmethod
    def create_minimizer(module_name, MC_norm, parameter):
        ...
        return classes[0][1](MC_norm, parameter)
    ...
    def get_minimizer_names():
        ...
        for full_module_name in glob(join(dirname(__file__), "*.py")):
            if isfile(full_module_name) and full_module_name != __file__:
                ...
                if not module_name.startswith('_') and module_name != 'abs_minimizer.py':
