...
class MMC(Minimizer):
    ...
    **settings : dict
    ...
    ----------------
    MC_norm : float
        Normalization parameter for MC which determines the accept/reject ratio. Defaults to `1.0`.
    ...
    def change_parameters(self) -> None:
        """
        Select a new value for each parameter from a distribution centered around the current value.

        .. note::

           For ``Parameter``s with ``constraints`` set, any proposed new value that would
    ...
    def has_converged(self) -> bool:
        """
        Check if the refinement process has converged on a stable solution.
    ...
    def reset_parameters(self) -> None:
        """
        Reset the ``Parameter`` values to the values from the previous MMC step.
        """
    ...
    def extract_result(self) -> 'list[str]':
        """
        Extract the result data from the history of the minimizer run.
    ...
    def format_result_string(self, minimizer_output: list) -> str:
        """
        Format a string output for the results of an MMC minimizer run.
