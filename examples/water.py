...
H1 = Atom("H")
H2 = Atom("H", position=(0.0, 1.63298, 0.0))
O = Atom("O", position=(0.0, 0.81649, 0.57736))
H_coulombic = Coulombic(atoms=[H1, H2], cutoff=10.0)
O_coulombic = Coulombic(atoms=O, cutoff=10.0)
water_mol = Molecule(
    position=(0, 0, 0),
    velocity=(0, 0, 0),
    atoms=[H1, H2, O],
    interactions=[Bond((H1, O), (H2, O), constrained=True), BondAngle(H1, O, H2, constrained=True)],
    name="water",
)
...
O_dispersion = Dispersion(
    universe, (O.atom_type, O.atom_type), cutoff=10.0, vdw_tail_correction=True
)
...
simulation = Simulation(
    universe, engine="lammps", time_step=1.033916924, temperature=280.0, traj_step=4000
)
