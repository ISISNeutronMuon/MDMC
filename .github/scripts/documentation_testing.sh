...
if [ ${{ env.GITHUB_EVENT_NAME }} != schedule ]
  ...
  elif git diff remotes/origin/${{ env.GITHUB_BASE_REF }} remotes/origin/${{ env.GITHUB_HEAD_REF }} -- requirements.txt | grep  '+ipython\|+ipykernel' # if there is a change to ipython which affects jupyter
  then
