...
awk 'BEGIN {
    ...
    exit pr_score < cron_score}' && echo "Your score is higher than (or equal to) master - linting passed!" && exit 0 || echo "Your score is lower than master - linting failed." && exit 1
