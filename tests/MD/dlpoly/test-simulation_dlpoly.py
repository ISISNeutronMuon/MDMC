...
#pytestmark = [pytest.mark.lammps] # TODO: find out what this is.
...
def dlpoly_universe(universe):
    ...
    #dlpoly_universe = lmp_eng.LAMMPSUniverse(universe)
...
@pytest.fixture
#def populated_lammps_simulation(universe, dlpoly_universe):
...
test_attr_set_get = [
    ("temperature", 5), # float
    ("pressure", 20), # float
    ("ensemble", 'nvt'), # DLPOLYEnsemble
]
@pytest.mark.parametrize("attr, val", test_attr_set_get,
...
def test_barostat_set_get(dlpoly_eng,attr,val):
    setattr(dlpoly_eng,"pressure",20)
    setattr(dlpoly_eng,"temperature",5)
    ic(dlpoly_eng.pressure)
    ic(dlpoly_eng.temperature)

    setattr(dlpoly_eng,attr,val)
    assert getattr(dlpoly_eng,attr) == val

#TODO: Fix this for multiple trajectories steps
...
@pytest.mark.parametrize("traj_hist_n_steps, n_steps, position", 
                         [(traj_hist_one_step,1,[[[1.,2.,3.],[4.,5.,6.]]]),
                          (traj_hist_two_steps,2,[[[2.,4.,6.],[8.,10.,12.]]])],
...
def test_convert_trajectory(dlpoly_eng,traj_hist_n_steps,n_steps,position):
    ...
    with tempfile.NamedTemporaryFile('w+',encoding='utf-8') as traj_file:
        ...
        traj = dlpoly_eng.convert_trajectory(0,1,1)
...
# Other functions: setup_universe, setup_simulation, update_parameter, save_config, reset_config