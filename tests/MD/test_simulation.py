...
def test_copy_structure(unit, changed_attr):

    """
    Tests that structure.copy copies the correct attributes and modifies
...
@parametrize('structure', [fixture_ref(atom), fixture_ref(water_molecule)])
def test_add_structure_center(universe, structure, request):

    """
    Tests that passing center=True to Universe.add_structure adds the
    structure to the center of the Universe
    """

    assert all(structure.position != universe.dimensions / 2)
    universe.add_structure(structure, center=True)
    assert all(structure.position == universe.dimensions / 2)
...
def test_spce_water_box(water_SPCE_universe):
    ...
    # water_positions = sorted([list(structure.position)
    #                           for structure
