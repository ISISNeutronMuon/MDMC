...
def GPR_with_history(parameters):
    ...
    minimizer = GPR(parameters, n_steps=10)
    for i in range(10):
...
def test_GPR_parameter_point_array_hypercube(constrained_parameters):
    ...
    gpr = GPR(constrained_parameters, n_steps=4)
    ...
    assert np.all(
        [
            np.array(points)[:, 0] >= par1_constraints[0],
    ...
    assert np.all(
        [
            np.array(points)[:, 1] >= par2_constraints[0],
...
def test_GPR_global_minimum_position(FoMs, coordinates, expected):
    ...
    constrained_par = Parameters([Parameter(name="parameter1", value=1.0, constraints=(0.5, 2.0))])
    gpr = GPR(constrained_par, n_steps=4)
...
def test_GPR_create_bounds():
    ...
    constrained_parameter = Parameter(name="parameter1", value=1.0, constraints=(0.5, 2.0))
    unconstrained_parameter = Parameter(name="parameter1", value=1.0)
    unconstrained_parameter_zero = Parameter(name="parameter1", value=0.0)

    gpr = GPR(Parameters(constrained_parameter), n_steps=4)
    ...
    with pytest.raises(ValueError):
        gpr.create_bounds(unconstrained_parameter_zero)


def test_GPR_set_parameter_values():
    """Tests set_parameter_values can set values correctly"""
    constrained_par = Parameters(
        [
            Parameter(name="parameter1", value=1.0, constraints=(0.5, 2.0)),
            Parameter(name="parameter2", value=2.0, constraints=(0.3, 6.0)),
        ]
    )
    gpr = GPR(constrained_par, n_steps=4)
...
def test_GPR_fit(parameters):
    ...
    mocked_df = pd.DataFrame(
        data=[[0, 100.0, 0.2, 2.6], [1, 150.5, 1.8, 2.6]],
        columns=["Unnamed: 0", "FoM", "epsilon", "sigma"],
    )

    with (
        patch("MDMC.refinement.minimizers.GPR.pd.read_csv", autospec=True, return_value=mocked_df),
        patch("MDMC.refinement.minimizers.GPR.skGPR.fit", autospec=True) as mock_fit):
        gpr = GPR(parameters, n_steps=4)
        _, _, _ = gpr.GPR_fit()
...
def test_GPR_minimizer_change_constrained_parameter():
    ...
    parameters = Parameters(
        [
            Parameter(name="constraints", value=1.0, constraints=(0.5, 1.5)),
            Parameter(name="constraints_2", value=1.0, constraints=(0.5, 1.5)),
        ]
    )
...
def test_converge_message_in_output(GPR_with_history, mocked_df, has_converged_value):
    ...
    with patch(
        "MDMC.refinement.minimizers.GPR.GPR.has_converged",
        autospec=True,
        return_value=has_converged_value,
    ):
        with patch(
            "MDMC.refinement.minimizers.GPR.pd.read_csv", autospec=True, return_value=mocked_df
        ):
