...
then  
  ...
  ! git diff remotes/origin/${TRAVIS_BRANCH} remotes/origin/${TRAVIS_PULL_REQUEST_BRANCH} -- requirements.txt | grep  '+ipython\|+ipykernel' # if there is a change to doc but not to ipython
  then
  ...
  elif git diff remotes/origin/${TRAVIS_BRANCH} remotes/origin/${TRAVIS_PULL_REQUEST_BRANCH} -- requirements.txt | grep  '+ipython\|+ipykernel' # if there is a change to ipython which affects jupyter
  then
